...
class DatabaseManagementController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        // Get database info based on driver
        $dbDriver = DB::getDriverName();
        $dbConfig = DB::getConfig();

        if ($dbDriver === 'sqlite') {
            $dbPath = database_path($dbConfig['database'] ?? 'database.sqlite');
            $dbInfo = [
                'driver' => $dbDriver,
            ...
            $dbInfo = [
                ...
                'exists' => $this->checkDatabaseConnection(),
            ];
        }
    ...
    protected function validateCurrentDatabase(): void
    {
        ...
        if ($dbDriver === 'sqlite') {
            ...
            if (($result[0]->integrity_check ?? '') !== 'ok') {
                throw new \RuntimeException('SQLite database integrity check failed');
            }
        } elseif ($dbDriver === 'mysql') {
            // For MySQL, just verify connection works
            try {
                DB::connection()->getPdo();
            } catch (\Exception $e) {
                throw new \RuntimeException('MySQL database connection check failed: ' . $e->getMessage());
            }
        } else {
            // For other drivers, just verify connection
            try {
                DB::connection()->getPdo();
            } catch (\Exception $e) {
