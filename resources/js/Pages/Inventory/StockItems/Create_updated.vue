<template>
    <AppLayout>
                    ...
                    <form @submit.prevent="submitNewDrug" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input v-model="newDrug.brand_name" type="text" required class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="e.g., Panadol"/>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Generic Name</label>
                                <input v-model="newDrug.generic_name" type="text" class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="e.g., Paracetamol"/>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select v-model="newDrug.category_id" required class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500" @change="onNewDrugCategoryChange">
                            ...
                            <div>
                                ...
                                <select v-model="newDrug.subcategory_id" class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500" :disabled="!newDrug.category_id">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input v-model="newDrug.strength" type="text" placeholder="e.g., 500mg" class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500"/>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Dosage Form</label>
                                <select v-model="newDrug.dosage_form" class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500">
                            ...
                            <div>
                                ...
                                <input v-model="newDrug.barcode" type="text" placeholder="e.g., 1234567890123" class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500"/>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input v-model="newDrug.manufacturer" type="text" class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="e.g., GlaxoSmithKline"/>
                            </div>
                            <div class="flex items-center pt-6">
                                <input v-model="newDrug.is_prescription_only" type="checkbox" id="prescription_only" class="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-400 rounded"/>
                ...
                <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
                    ...
                    <form @submit.prevent="submitNewCategory" class="space-y-4">
                        <div>
                            ...
                            <input v-model="newCategory.name" type="text" required class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="e.g., Pharmaceuticals"/>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                            <textarea v-model="newCategory.description" rows="2" class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="Optional description"></textarea>
                        </div>
                ...
                <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
                    ...
                    <form @submit.prevent="submitNewSubcategory" class="space-y-4">
                        <div>
                            ...
                            <input type="text" :value="selectedCategoryName" disabled class="w-full rounded-md border-gray-400 bg-gray-100 shadow-sm"/>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Subcategory Name *</label>
                            <input v-model="newSubcategory.name" type="text" required class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="e.g., Antibiotics"/>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                            <textarea v-model="newSubcategory.description" rows="2" class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="Optional description"></textarea>
                        </div>
        ...
        <!-- Edit Category Modal -->
        <div v-if="showEditCategoryModal" class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen px-4">
                <div class="fixed inset-0 bg-gray-500 bg-opacity-75" @click="showEditCategoryModal = false"></div>
                <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-semibold text-gray-900">Edit Category</h3>
                        <button type="button" @click="showEditCategoryModal = false" class="text-gray-400 hover:text-gray-500">
                            <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                            </svg>
                        </button>
                    ...
                    <form @submit.prevent="submitEditCategory" class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Category Name *</label>
                            <input v-model="editCategoryForm.name" type="text" required class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="e.g., Pharmaceuticals"/>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                            <textarea v-model="editCategoryForm.description" rows="2" class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="Optional description"></textarea>
                        </div>

                        <div class="flex justify-end gap-3 pt-4">
                            <button type="button" @click="showEditCategoryModal = false" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">
                                Cancel
                            </button>
                            <button type="submit" :disabled="editCategoryLoading" class="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700 disabled:opacity-50">
        ...
        <div v-if="showEditSubcategoryModal" class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen px-4">
                <div class="fixed inset-0 bg-gray-500 bg-opacity-75" @click="showEditSubcategoryModal = false"></div>
                <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg font-semibold text-gray-900">Edit Subcategory</h3>
                        <button type="button" @click="showEditSubcategoryModal = false" class="text-gray-400 hover:text-gray-500">
                            <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                            </svg>
                        </button>
                    ...
                    <div v-if="editSubcategoryError" class="mb-4 p-3 bg-red-50 border border-red-200 rounded-md">
                        <p class="text-sm text-red-600">{{ editSubcategoryError }}</p>
                    </div>
                    ...
                    <form @submit.prevent="submitEditSubcategory" class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Subcategory Name *</label>
                            <input v-model="editSubcategoryForm.name" type="text" required class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="e.g., Antibiotics"/>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
                            <textarea v-model="editSubcategoryForm.description" rows="2" class="w-full rounded-md border-gray-400 shadow-sm focus:border-primary-500 focus:ring-primary-500" placeholder="Optional description"></textarea>
                        </div>

                        <div class="flex justify-end gap-3 pt-4">
                            <button type="button" @click="showEditSubcategoryModal = false" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50">
                                Cancel
                            </button>
                            <button type="submit" :disabled="editSubcategoryLoading" class="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-md hover:bg-primary-700 disabled:opacity-50">
                                {{ editSubcategoryLoading ? 'Updating...' : 'Update Subcategory' }}
                            </button>
                        </div>
                    </form>
                </div>
...
const lookupBarcode = async () => {
    ...
    try {
        const response = await axios.get(route('api.barcode.lookup', { barcode: barcodeSearch.value }));

        if (response.data.found && response.data.drug) {
            handleProductSelect(response.data.drug);
            barcodeSuccess.value = `Found: ${response.data.drug.brand_name}`;
            barcodeSearch.value = '';
        } else {
...
const submitNewDrug = async () => {
    ...
    try {
        const response = await axios.post(route('api.drugs.store'), newDrug.value);
        ...
        if (response.data.drug) {
            ...
            newDrugError.value = response.data.message || 'Failed to create product';
        }
    } catch (error) {
        console.error('Drug creation error:', error);
        if (error.response?.data?.errors) {
            const firstError = Object.values(error.response.data.errors)[0];
            newDrugError.value = Array.isArray(firstError) ? firstError[0] : firstError;
        } else if (error.response?.data?.message) {
            newDrugError.value = error.response.data.message;
        } else {
            newDrugError.value = 'Network error. Please check your connection.';
        }
    } finally {
...
const submitNewCategory = async () => {
    ...
    try {
        const response = await axios.post(route('api.categories.store'), newCategory.value);

        if (response.data.category) {
            // Add to local categories
            localCategories.value.push(response.data.category);
            // Select the new category
            selectedCategory.value = response.data.category.id;
            selectedSubcategory.value = '';
            // Close modal and reset form
            showNewCategoryModal.value = false;
            newCategory.value = { name: '', description: '' };
        } else {
            newCategoryError.value = response.data.message || 'Failed to create category';
        }
    } catch (error) {
        console.error('Category creation error:', error);
        if (error.response?.data?.errors) {
            const firstError = Object.values(error.response.data.errors)[0];
            newCategoryError.value = Array.isArray(firstError) ? firstError[0] : firstError;
        } else if (error.response?.data?.message) {
            newCategoryError.value = error.response.data.message;
        } else {
            newCategoryError.value = 'Network error. Please check your connection.';
        }
    } finally {
...
const submitNewSubcategory = async () => {
    ...
    try {
        const response = await axios.post(route('api.subcategories.store'), {
        ...
        if (response.data.subcategory) {
            ...
            if (categoryIndex !== -1) {
                ...
                localCategories.value[categoryIndex].subcategories.push(response.data.subcategory);
            }
            // Select the new subcategory
            selectedSubcategory.value = response.data.subcategory.id;
            // Close modal and reset form
            showNewSubcategoryModal.value = false;
            newSubcategory.value = { name: '', description: '' };
        } else {
            newSubcategoryError.value = response.data.message || 'Failed to create subcategory';
        }
    } catch (error) {
        console.error('Subcategory creation error:', error);
        if (error.response?.data?.errors) {
            const firstError = Object.values(error.response.data.errors)[0];
            newSubcategoryError.value = Array.isArray(firstError) ? firstError[0] : firstError;
        } else if (error.response?.data?.message) {
            newSubcategoryError.value = error.response.data.message;
        } else {
            newSubcategoryError.value = 'Network error. Please check your connection.';
        }
    } finally {
...
const submitEditCategory = async () => {
    ...
    try {
        ...
        if (error.response?.data?.errors) {
            const firstError = Object.values(error.response.data.errors)[0];
            editCategoryError.value = Array.isArray(firstError) ? firstError[0] : firstError;
        } else if (error.response?.data?.message) {
            editCategoryError.value = error.response.data.message;
        } else {
            editCategoryError.value = 'Failed to update category';
        }
    } finally {
...
const editSubcategory = () => {
    ...
    const subcategory = category.subcategories?.find(s => s.id === selectedSubcategory.value);
    if (subcategory) {
        ...
        editSubcategoryError.value = '';
    }
};

const submitEditSubcategory = async () => {
    editSubcategoryLoading.value = true;
    editSubcategoryError.value = '';

    try {
        const response = await axios.put(
            route('api.subcategories.update', editSubcategoryForm.value.id),
            {
                name: editSubcategoryForm.value.name,
                description: editSubcategoryForm.value.description
            }
        );

        if (response.data.success) {
            // Update local subcategory
            const category = localCategories.value.find(c => c.id === selectedCategory.value);
            if (category && category.subcategories) {
                const index = category.subcategories.findIndex(s => s.id === editSubcategoryForm.value.id);
                if (index !== -1) {
                    category.subcategories[index] = response.data.subcategory;
                }
            }
        ...
        if (error.response?.data?.errors) {
            const firstError = Object.values(error.response.data.errors)[0];
            editSubcategoryError.value = Array.isArray(firstError) ? firstError[0] : firstError;
        } else if (error.response?.data?.message) {
            editSubcategoryError.value = error.response.data.message;
        } else {
            editSubcategoryError.value = 'Failed to update subcategory';
        }
    } finally {
        editSubcategoryLoading.value = false;
    }
};
...
const deleteSubcategory = async () => {
    ...
    try {
        const response = await axios.delete(route('api.subcategories.delete', subcategory.id));

        if (response.data.success) {
            // Remove from local subcategories
